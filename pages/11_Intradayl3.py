...
CHAT_ID = "299717233" # Chat ID se koi khatra nahi hai, ise rehne de sakte hain

# 🔴 YAHAN APNI GOOGLE SHEET KA ASLI URL PASTE KAREIN
...
def get_google_sheet():
    ...
    # Cloud aur Local dono jagah chalne ke liye master logic
    if "gcp_service_account" in st.secrets:
        # Jab aap Streamlit Cloud par upload karenge
        creds = ServiceAccountCredentials.from_json_keyfile_dict(st.secrets["gcp_service_account"], scope)
    else:
        # Jab tak aap computer par test kar rahe hain
        if not os.path.exists("creds.json"):
            st.error("⚠️ 'creds.json' file nahi mili! Please check Step 2.")
            return None
    ...
    try:
        ...
        st.error(f"⚠️ Sheet Open Error: {e}")
        return None

def load_sheet_data():
    sheet = get_google_sheet()
    if sheet is None: return pd.DataFrame(columns=['Symbol', 'Base_Price'])
...
def send_telegram_alert(message):
    ...
    for index, chunk in enumerate(chunks):
        ...
        try: requests.post(url, json={"chat_id": CHAT_ID, "text": final_text}, timeout=15)
        except: pass
...
if st.sidebar.button("💾 Save Changes To Google Sheet", type="primary"):
    ...
    if sheet:
        with st.spinner("Google Sheet Syncing..."):
            # Format symbols securely
            if not edited_watchlist.empty and 'Symbol' in edited_watchlist.columns:
                edited_watchlist['Symbol'] = edited_watchlist['Symbol'].apply(lambda x: str(x).upper().strip() + ".NS" if pd.notna(x) and not (str(x).upper().endswith(".NS") or str(x).upper().endswith(".BO")) else x)
            
            # Clear old data and write fresh rows securely
            sheet.clear()
...
if not st.session_state.live_df.empty:
    ...
    if search_q: display_df = display_df[display_df['Symbol'].str.contains(search_q)]
    if sort_action == "Max to Min % Change": display_df = display_df.sort_values(by="% Change", ascending=False)
    elif sort_action == "Min to Max % Change": display_df = display_df.sort_values(by="% Change", ascending=True)
